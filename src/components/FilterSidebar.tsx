...
export function FilterSidebar({ isOpen, onClose }: FilterSidebarProps) {
  ...
  const activeFiltersCount = selectedCategories.length + selectedBrands.length + (((priceRange[0] ?? 0) > 0 || (priceRange[1] ?? 1000) < 1000) ? 1 : 0);
  ...
  return (
    <div className="fixed inset-0 z-40 lg:relative lg:inset-auto">
      ...
      {/* Sidebar */}
      <div className="fixed lg:relative right-0 lg:right-auto top-0 h-full w-80 bg-white shadow-xl lg:shadow-lg lg:rounded-xl overflow-y-auto animate-slide-in-right lg:animate-none">
        <div className="p-6">
          {/* Header */}
          <div className="flex items-center justify-between mb-6 pb-4">
            ...
            <div className="flex space-x-2">
              {activeFiltersCount > 0 && (
                <Button variant="ghost" size="sm" onClick={clearFilters} className="text-gray-600 hover:text-primary">
                  Clear All
                </Button>
              )}
          ...
          {/* Categories */}
          <Card className="mb-6 border-0 shadow-md bg-gray-50/50">
            <CardHeader 
              className="pb-3 cursor-pointer hover:bg-gray-100/50 rounded-t-lg transition-colors"
